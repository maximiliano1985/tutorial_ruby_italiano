...
x = 10
x_binario = x.to_s(2)
puts "x = #{x} in binario e' #{ x_binario }"
x_esadecimale = x.to_s(16)
puts "x = #{x} in esadecimale e' 0x#{ x_esadecimale }"
# per tornare indietro posso usare il metodo .hex
x_decimale = x_esadecimale.hex
puts "torno indietro, x_esadecimale #{x_esadecimale} in decimale e' #{x_decimale} "
puts ""
...
num = 7
num_bin = num.to_s 2
puts "#{num} in binario e' #{num_bin}"
num_unpack = num.to_s.unpack("B*")
puts "#{num} convertito con unpack in binario e' #{num_unpack}"
# NOTA CHE HO DUE RISULTATI DIVERSI, QUESTO e' PERCHe' UNPACK TRATTA NUM COME SE FOSSE UN CARATTERE ASCII E NON UN NUMERO DECIMALE! INFATTI MI RESTUISCE "00110011" CHE CORRISPONDE ALLA RAPPRESENTAZIONE BINARIA DI 51. VERIFICO:
num_8bit = num.to_s.unpack("U*")
puts "#{num} convertito con unpack in carattere a 8 bit e' #{num_8bit}"
