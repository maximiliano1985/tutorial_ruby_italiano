...
def hanoi(num_dischi, start=0, target=2, usando=1)
  movimenti = []
  ...
  if num_dischi == 1
    ...
    movimenti << [start, target]
  else
    movimenti.concat hanoi(num_dischi-1, start, usando, target)
    movimenti.concat hanoi(1,            start, target, usando)
    movimenti.concat hanoi(num_dischi-1, usando, target, start)
  end
  return movimenti
end
